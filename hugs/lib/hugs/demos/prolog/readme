...
Mini Prolog Version 1.5g             A simple Prolog interpreter, for Hugs 1.3

     Mark P. Jones, 23rd July 1991, updated for Hugs 1.3, June 1996.
______________________________________________________________________________

This document gives a brief introduction to Mini Prolog Version 1.5g, a simple
Prolog interpreter that can be used with Hugs 1.3.  The original version of
this program was written nearly two years ago as an Orwell program.  It has
been through many minor changes since then, modified to run under Haskell B,
then Gofer, and now Hugs.
...
The  Mini Prolog interpreter  takes the form of a small  collection  of Hugs
...
the Hugs interpreter, and that all of the Mini Prolog script files are in the
current working directory, you should start Hugs with the command `hugs':

  C:\HUGS\DEMOS\PROLOG>hugs
   ...
   /__/   /__/  /_________/  /_________/  /_________/          Release alpha

       Copyright (c) Mark P Jones, The University of Nottingham, 1994-1996.

  Reading script file "\Hugs\Lib\hugs.prelude":

  Hugs session for:
  \Hugs\Lib\hugs.prelude
  Type :? for help
  ?

and then load the files for the Mini Prolog system:
...
The `?' prompt  tells you that you are now back in Hugs,  and you can restart
Mini Prolog as before,  carry on with some other work in Hugs,  or use the :q
command to exit Hugs and return to the operating system.
